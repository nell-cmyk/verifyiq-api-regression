...
| `monitoring` | 69 | Yes | opt-in GET smoke on safe no-path and setup-backed detail endpoints across overview, request detail, golden-dataset, ground-truth, GCS, and drift routes | smoke, contract, auth, negative | Medium | Partially covered | 38 monitoring GET endpoints now return 200 in smoke, including request detail/document/file/golden-status routes, golden-dataset detail/file, ground-truth schema, drift trends, and GCS variants/documents/preview. Deferred: `timeseries/{metric}`, `providers`, and `gcs/structure`. |
| `parser-studio` | 43 | Yes | opt-in GET smoke on safe current no-path and setup-backed detail/version endpoints | smoke, contract, auth, negative | Medium | Partially covered | Covered now: `auth/status`, current v1 metadata/defaults/audit/tenants/fraud-thresholds routes, `/parser_studio/api/v1/document-types/{doc_type}`, `/versions`, `/prompt/versions`, and `/tenants/{api_key}`. Legacy unversioned aliases are excluded. Deferred: `/prompt/versions/{version_id}` still lacks a version id from the current prompt-version response. |
...
- `/parser_studio/api/v1/document-types/{doc_type}/prompt/versions/{version_id}` still lacks a version id because the current prompt-versions response returned an empty `versions` list for the repo-derived active document type
- `/monitoring/api/v1/timeseries/{metric}` still lacks a repo-backed metric inventory; ad hoc metric probes are not yet strong enough to promote a single metric into smoke
