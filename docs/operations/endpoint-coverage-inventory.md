...
- Split-readiness note: future `extended` work for this group is docs/planning/dry-run only and must keep `tools/run_regression.py` canonical, `protected` parse-only, and current `smoke` coverage preserved until parity and approval gates are complete. Candidate planning is limited to the already smoke-covered read-only/status and setup-backed GET subset with list-derived `applicationId` and document id aliases; export, page, document, and activity responses are sensitive and future hub evidence should remain metadata-only unless owner-approved body retention exists. Mutating, search, and application-management routes stay out of scope or blocked, and this note does not approve live `extended`, smoke replacement, CI expansion, manifest nodes, runner behavior, tests, OpenAPI changes, generated files, or reports.
...
- Split-readiness note: future `extended` work for this group is docs/planning/dry-run only and must keep current `smoke` coverage preserved until parity and approval gates are complete. Candidate planning is limited to the already smoke-covered QA API GET subset; queue-derived correlation IDs must be modeled as sensitive aliases or named outputs, not raw response-body coupling, and review-detail `404` responses are candidate-selection misses rather than covered endpoint success. Future hub evidence should remain metadata-only unless owner-approved retention exists, and raw queue, request, review, report, threshold, export, and export-preview bodies must not be persisted without owner-approved retention. The `/qa` UI surface and QA POST/PUT or other mutation routes stay out of scope or blocked, and this note does not approve live `extended`, smoke replacement, CI expansion, manifest nodes, runner behavior, tests, OpenAPI changes, generated files, reports, or live execution changes.
...
- Split-readiness note: future `extended` work for this group is docs/planning/dry-run only and must keep current `smoke` coverage preserved until parity and approval gates are complete. Candidate planning is limited to the already smoke-covered current v1 read-only/status and setup-backed GET subset, with dependencies on active `document_type`, prompt `version_id`, and tenant `api_key`; sensitive dependency values must be modeled as safe aliases or named outputs, not raw response-body coupling. Malformed setup data fails loudly, and missing usable prerequisites may skip only through the existing helper/test semantics. Future hub evidence must remain metadata-only unless owner-approved retention exists, and raw document-type, prompt, audit, tenant, fraud-threshold, and configuration bodies must not be persisted without owner-approved retention. Legacy unversioned aliases, UI routes, auth login/logout, mutating Parser Studio routes, and workflow-style operations stay excluded or blocked; no owner confirmation currently promotes this group into protected/default coverage or live `extended`, and this note does not approve live `extended`, live `--hub-group`, smoke replacement, CI expansion, manifest nodes, runner behavior, tests, OpenAPI changes, generated files, reports, body persistence, or live execution changes.
...
- Split-readiness note: future `extended` work for this mixed group is docs/planning/dry-run only and must keep current `smoke` coverage preserved until parity and approval gates are complete. Candidate planning is limited to the already smoke-covered gateway health, benchmark list/detail, and PII cleanup runs GET subset; benchmark `job_id` dependencies must be modeled as safe aliases or named outputs, not raw response-body coupling. Future hub evidence must remain metadata-only unless owner-approved retention exists, and raw gateway, benchmark, PII cleanup, storage, file-path, or utility bodies must not be persisted without owner-approved retention. Gateway S3 list/download/presigned-url routes, legacy AI shortcut routes, debug routes, mutating gateway/application/benchmark/PII cleanup routes, storage-risk surfaces, and workflow-style operations stay deferred, excluded, or blocked; no owner confirmation currently promotes this mixed group into protected/default coverage or live `extended`, and this note does not approve live `extended`, live `--hub-group`, smoke replacement, CI expansion, manifest nodes, runner behavior, tests, OpenAPI changes, generated files, reports, body persistence, or live execution changes.
