...
| `document-processing-adjacent` | 4 | No | none | smoke, contract, negative | High | Proposed/blocked | Proposed first expansion tranche is `GET /v1/documents/fraud-status/{job_id}` as a setup-backed, read-only fraud-detection status check. External API source evidence now identifies strict fraud job-id format, tenant-scoped lookup, status response shapes, and the async fraud parse path as the likely producer, but the non-live plan in `docs/knowledge-base/document-processing-adjacent/fraud-status-expansion-plan.md` keeps it blocked until an API owner confirms setup safety, staging prerequisites, artifact policy, and runner lane. `/v1/documents/check-cache`, `/v1/documents/crosscheck`, and mutating `/v1/documents/cache` remain deferred pending request-shape, fixture, artifact, and owner review. |
