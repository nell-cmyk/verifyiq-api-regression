...
| `document-processing-adjacent` | 4 | No | none | smoke, contract, negative | High | Proposed/blocked | Proposed first expansion tranche is `GET /v1/documents/fraud-status/{job_id}` as a setup-backed, read-only fraud-detection status check. The non-live plan in `docs/knowledge-base/document-processing-adjacent/fraud-status-expansion-plan.md` keeps it blocked until an API owner confirms the fresh job-id producer, lifecycle statuses, auth behavior, stable contract fields, artifact policy, and runner lane. `/v1/documents/check-cache`, `/v1/documents/crosscheck`, and mutating `/v1/documents/cache` remain deferred pending request-shape, fixture, artifact, and owner review. |
...
- `/v1/documents/fraud-status/{job_id}` requires a fresh fraud job id; benchmark job ids and monitoring request-derived identifiers returned `404` with `"Fraud job not found or expired."`, and the repo does not expose a producer flow for a fresh fraud job id. It is the proposed first document-processing-adjacent expansion candidate, but only after the producer, owner, lifecycle statuses, contract fields, auth expectations, artifact policy, and runner lane are confirmed. See `docs/knowledge-base/document-processing-adjacent/fraud-status-expansion-plan.md`.
