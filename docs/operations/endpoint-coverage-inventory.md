...
| `document-processing-adjacent` | 4 | No | none | smoke, contract, negative | High | Proposed | Proposed first expansion tranche is `GET /v1/documents/fraud-status/{job_id}` as a setup-backed, read-only fraud-detection status check once an approved fresh job-id producer exists. `/v1/documents/check-cache`, `/v1/documents/crosscheck`, and mutating `/v1/documents/cache` remain deferred pending request-shape, fixture, artifact, and owner review. |
