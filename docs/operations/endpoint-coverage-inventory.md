...
- `/v1/documents/fraud-status/{job_id}` requires a fresh fraud job id; benchmark job ids and monitoring request-derived identifiers returned `404` with `"Fraud job not found or expired."`, and the repo does not expose a producer flow for a fresh fraud job id
