...
| `batch` | 1 | Yes | happy-path, selective contract, validation, negative, partial-failure, safe-limit handling | smoke/protected candidate, contract, auth, negative | High | Partially covered | Representative auth-negative coverage is still missing. Default live selection is capped at 4 items. |
...
- Close the remaining `batch` auth-negative gap.
