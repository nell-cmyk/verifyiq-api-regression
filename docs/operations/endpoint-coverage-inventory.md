...
- Current approved live Automation Hub node: `./.venv/bin/python tools/run_regression.py --suite extended --hub-node get-smoke.health.core`
- `smoke` is now real, but it is not a broader current default
- broad live `--suite extended`, live `--hub-group`, and all other hub nodes remain blocked
...
| `health` | 5 | Yes | opt-in GET smoke on top-level `/health*` 200 routes plus exact-status checks for known admin-key surfaces; approved hub node `get-smoke.health.core` for `GET /health` only | smoke, basic contract | Medium | Covered | The top-level health group is covered through `./.venv/bin/python tools/run_regression.py --suite smoke`, and the first live-capable Automation Hub tranche is limited to `./.venv/bin/python tools/run_regression.py --suite extended --hub-node get-smoke.health.core`. Health-like `/api/v1/applications/health`, `/api/v1/admin/health`, and `/ai-gateway/health/gateway-circuit-breakers` are covered through neighboring groups. `/health/ready` and the other `/health/*` routes remain outside the approved live hub node. `/api/v1/health/database-pools` and `/api/v1/health/database-pools/metrics` are codified in smoke as expected `401` responses rather than treated as unresolved 200 targets. |
...
- The current non-live `--suite extended --dry-run` preview models dependency order and named output/input relationships only; it does not execute endpoints or prove live safety. Live hub execution is approved only for `get-smoke.health.core`, representing `GET /health` only.
