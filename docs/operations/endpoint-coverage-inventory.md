...
| `document-processing-adjacent` | 4 | No | none | smoke, contract, negative | High | Proposed/blocked | Proposed first expansion tranche is `GET /v1/documents/fraud-status/{job_id}` as a setup-backed, read-only fraud-detection status check. External API source evidence identifies strict fraud job-id format, tenant-scoped lookup, status response shapes, and the async fraud parse path as the likely producer. Observed in bounded staging characterization on 2026-04-27, the async parse producer returned a format-valid fraud job ID, bounded polling reached `complete`, and invalid-format plus artificial valid nonexistent job IDs returned `404`. This is a provisional regression contract, not owner-confirmed; do not promote to covered until behavior is stable and artifact policy remains safe. `/v1/documents/check-cache`, `/v1/documents/crosscheck`, and mutating `/v1/documents/cache` remain deferred pending request-shape, fixture, artifact, and owner review. |
| `health` | 5 | Yes | opt-in GET smoke on top-level `/health*` 200 routes plus exact-status checks for known admin-key surfaces | smoke, basic contract | Medium | Covered | The top-level health group is now covered through `./.venv/bin/python tools/run_regression.py --suite smoke`. Health-like `/api/v1/applications/health`, `/api/v1/admin/health`, and `/ai-gateway/health/gateway-circuit-breakers` are covered through neighboring groups. `/api/v1/health/database-pools` and `/api/v1/health/database-pools/metrics` are now codified in smoke as expected `401` responses rather than treated as unresolved 200 targets. |
...
| `monitoring` | 69 | Yes | opt-in GET smoke on safe no-path and setup-backed detail endpoints across overview, request detail, timeseries, golden-dataset, ground-truth, GCS, and drift routes, plus one exact-status guard | smoke, contract, auth, negative | Medium | Partially covered | 42 monitoring GET endpoints now return 200 in smoke, including request detail/document/file/golden-status routes, the supported `timeseries` metrics, golden-dataset detail/file, ground-truth schema, drift trends, and GCS variants/documents/preview. Deferred: `providers` still lacks a legitimate 200 path. `/monitoring/api/v1/golden-dataset/gcs/structure` is now codified in smoke as the expected `502` response rather than treated as an unresolved 200 target. |
...
- `/v1/documents/fraud-status/{job_id}` requires a fresh tenant-scoped fraud job id. External API source evidence says valid IDs use a strict `fj_` plus 32 lowercase-hex format; malformed, missing, expired, and wrong-tenant jobs return `404` with `"Fraud job not found or expired."` Observed in bounded staging characterization on 2026-04-27, one protected-fixture parse request with `pipeline.async_fraud=true` produced a format-valid fraud job ID, bounded polling reached `complete`, and invalid-format plus artificial valid nonexistent job IDs returned `404`. The likely producer is async fraud scheduling from a parse request, but scheduling can fall back to synchronous fraud and return no job ID. Keep this proposed first document-processing-adjacent expansion candidate blocked until setup safety, staging prerequisites, artifact policy, runner lane, and the not owner-confirmed provisional regression contract are accepted. See `docs/knowledge-base/document-processing-adjacent/fraud-status-expansion-plan.md`.
