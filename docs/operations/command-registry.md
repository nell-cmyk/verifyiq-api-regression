...
| `./.venv/bin/python tools/reporting/plan_batch_ground_truth_recovery.py --run-dir reports/batch_ground_truth/batch_ground_truth_<timestamp> --reference-workbook /absolute/path/to/reference.xlsx` | Non-live planner that reads a previous `recovery_triage.csv`, selects only retryable recovery classes, and prints conservative targeted exporter commands with repeated `--file-type` arguments | terminal-only command plan | no repo mutation |
