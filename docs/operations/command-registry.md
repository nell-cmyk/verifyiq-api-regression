...
| `./.venv/bin/python tools/run_regression.py --suite extended --hub-node get-smoke.health.ready` | Approved live Automation Hub readiness-node execution; calls only `GET /health/ready`; broad `--suite extended`, live `--hub-group`, and other hub nodes remain blocked | Live API/IAP env: `BASE_URL`, `TENANT_TOKEN`, `API_KEY`, `IAP_CLIENT_ID`, `GOOGLE_APPLICATION_CREDENTIALS` | `reports/hub/<run-id>/run.json`, `run.md`, `reports/hub/LATEST.txt` | generated metadata-only artifacts |
