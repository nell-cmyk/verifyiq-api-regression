...
| `./.venv/bin/python tools/run_regression.py --suite extended --dry-run [--hub-node NODE_ID | --hub-group ENDPOINT_GROUP]` | Non-live Automation Hub dependency graph and reporting-contract preview, optionally filtered to a manifest slice plus prerequisites; live `--suite extended` is not implemented | `.venv` only; no live secrets | none | no repo mutation |
...
- `tools/run_regression.py --suite extended --dry-run` is a non-live Automation Hub preview only. It renders dependency order, named outputs/inputs, skip semantics, and the future reporting/redaction contract, but it does not execute endpoints or write reports. Add either `--hub-node NODE_ID` or `--hub-group ENDPOINT_GROUP` to filter the preview to a bounded manifest slice plus required prerequisites; selectors are discovery-only and do not prove live endpoint safety.
