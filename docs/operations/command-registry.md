...
- The first approved live Automation Hub node is `get-smoke.health.core`, reached through `./.venv/bin/python tools/run_regression.py --suite extended --hub-node get-smoke.health.core`; it requires live API/IAP env, calls only `GET /health`, and writes metadata-only `reports/hub/` artifacts.
...
| `./.venv/bin/python tools/run_regression.py --suite extended --dry-run [--report] [--hub-node NODE_ID | --hub-group ENDPOINT_GROUP]` | Non-live Automation Hub dependency graph and reporting-contract preview, optionally filtered to a manifest slice plus prerequisites; `--report` writes synthetic non-live plan evidence only | `.venv` only; no live secrets | optional `reports/hub/<run-id>/run.json`, `run.md`, `reports/hub/LATEST.txt` | generated artifacts only when `--report` is used |
| `./.venv/bin/python tools/run_regression.py --suite extended --hub-node get-smoke.health.core` | Approved live Automation Hub health-node execution; calls only `GET /health`; broad `--suite extended`, live `--hub-group`, and other hub nodes remain blocked | Live API/IAP env: `BASE_URL`, `TENANT_TOKEN`, `API_KEY`, `IAP_CLIENT_ID`, `GOOGLE_APPLICATION_CREDENTIALS` | `reports/hub/<run-id>/run.json`, `run.md`, `reports/hub/LATEST.txt` | generated metadata-only artifacts |
...
| `./.venv/bin/python tools/run_regression.py --suite smoke` | Canonical opt-in live GET smoke suite across safely testable VerifyIQ API GET endpoints, including bounded artifact-free fraud-status setup coverage | Live API env plus protected parse fixture env | none | no repo mutation |
...
| `./.venv/bin/python tools/run_regression.py --endpoint parse --category contract` | Canonical focused `/parse` response and validation contract selection backed by existing protected-suite tests | Live `/parse` env | `reports/parse/responses/parse_<timestamp>/...` | generated artifacts only |
| `./.venv/bin/python tools/run_regression.py --endpoint parse --category auth` | Canonical focused `/parse` tenant-token auth-negative selection backed by existing protected-suite tests | Live `/parse` env | `reports/parse/responses/parse_<timestamp>/...` | generated artifacts only |
| `./.venv/bin/python tools/run_regression.py --endpoint parse --category negative` | Canonical focused `/parse` missing-payload validation selection backed by existing protected-suite tests | Live `/parse` env | none | no repo mutation |
...
| `./.venv/bin/python tools/run_regression.py --endpoint batch --category contract` | Canonical focused `/documents/batch` top-level and per-item contract selection backed by existing batch-suite tests | Live `/documents/batch` env | `reports/batch/batch_<timestamp>/...` | generated artifacts only |
| `./.venv/bin/python tools/run_regression.py --endpoint batch --category negative` | Canonical focused `/documents/batch` validation, over-limit, and partial-failure selection backed by existing batch-suite tests | Live `/documents/batch` env | `reports/batch/batch_<timestamp>/...` | generated artifacts only |
...
| `./.venv/bin/python tools/safe_git_commit.py --message "Describe the reviewed change"` | Guarded commit flow after review; defaults to live protected baseline validation | Reviewed diff, staged changes, clean worktree | none | Git state only |
| `./.venv/bin/python tools/safe_git_commit.py --message "Describe the reviewed change" --validation non-live` | Guarded commit flow for documentation/tooling changes where live API validation is intentionally out of scope | Reviewed diff, staged changes, clean worktree; `.venv` only; no live secrets | none | Git state only |
...
| `VERIFYIQ_SKIP_DOTENV=1 ./.venv/bin/python tools/reporting/openapi_runtime_drift.py [--json]` | Non-live observed-runtime-baseline drift report comparing curated safe `/parse`, `/documents/batch`, provisional fraud-status, and provisional monitoring GCS types response shapes against `official-openapi.json`; findings are observed drift, not owner-approved public contract | terminal-only report | no repo mutation |
...
- `tools/run_regression.py --suite extended --dry-run` is a non-live Automation Hub preview. It renders dependency order, named outputs/inputs, skip semantics, and the future reporting/redaction contract, but it does not execute endpoints. Add either `--hub-node NODE_ID` or `--hub-group ENDPOINT_GROUP` to filter the preview to a bounded manifest slice plus required prerequisites; selectors are discovery-only and do not prove live endpoint safety. Add `--report` to write synthetic plan evidence under `reports/hub/`; the generated report contains no live endpoint results, request bodies, response bodies, raw document identifiers, GCS object names, fraud results, or artifact/export payloads.
- `tools/run_regression.py --suite extended --hub-node get-smoke.health.core` is the only approved live Automation Hub selector. It writes metadata-only hub reports with node id, `GET /health` label, status code, content-type, timing, outcome, and rerun selector; it does not persist request bodies, response bodies, auth headers, tokens, cookies, tenant/API keys, raw document/GCS identifiers, fraud fields, or artifact/export payloads. Do not run it unless live validation is explicitly approved.
...
- `tools/reporting/openapi_runtime_drift.py` treats curated current response shapes as observed runtime baselines for spec drift comparison. It covers `/parse`, `/documents/batch` envelope-only evidence, provisional fraud-status top-level evidence, and provisional monitoring GCS types top-level evidence; keeps `/documents/batch` `results[].data`, fraud-status terminal result fields, GCS category values, and child GCS endpoint shapes loose; and classifies other endpoint groups by readiness instead of running live calls.
