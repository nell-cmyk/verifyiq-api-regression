...
| `./.venv/bin/python tools/run_regression.py --suite extended --dry-run [--report] [--hub-node NODE_ID | --hub-group ENDPOINT_GROUP]` | Non-live Automation Hub dependency graph and reporting-contract preview, optionally filtered to a manifest slice plus prerequisites; `--report` writes synthetic non-live plan evidence only; live `--suite extended` is not implemented | `.venv` only; no live secrets | optional `reports/hub/<run-id>/run.json`, `run.md`, `reports/hub/LATEST.txt` | generated artifacts only when `--report` is used |
