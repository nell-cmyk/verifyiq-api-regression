...
| `VERIFYIQ_SKIP_DOTENV=1 ./.venv/bin/python tools/reporting/openapi_runtime_drift.py [--json]` | Non-live observed-runtime-baseline drift report comparing curated safe `/parse`, `/documents/batch`, and provisional fraud-status response shapes against `official-openapi.json`; findings are observed drift, not owner-approved public contract | terminal-only report | no repo mutation |
...
- `tools/reporting/openapi_runtime_drift.py` treats curated current response shapes as observed runtime baselines for spec drift comparison. It covers `/parse`, `/documents/batch` envelope-only evidence, and provisional fraud-status top-level evidence; keeps `/documents/batch` `results[].data` and fraud-status terminal result fields loose; and classifies other endpoint groups by readiness instead of running live calls.
