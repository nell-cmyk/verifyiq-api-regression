...
| `./.venv/bin/python tools/run_regression.py --suite extended --dry-run` | Non-live Automation Hub dependency graph and reporting-contract preview; live `--suite extended` is not implemented | `.venv` only; no live secrets | none | no repo mutation |
...
- `tools/run_regression.py --suite extended --dry-run` is a non-live Automation Hub preview only. It renders dependency order, named outputs/inputs, skip semantics, and the future reporting/redaction contract, but it does not execute endpoints or write reports.
