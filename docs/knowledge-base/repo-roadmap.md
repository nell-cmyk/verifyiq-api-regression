...
- Current OpenAPI drift comparison treats curated observed endpoint response shape as the runtime baseline against `official-openapi.json`, while keeping public API contract status separate from both observed behavior and the checked-in spec. The non-live `tools/reporting/openapi_runtime_drift.py` report currently compares `/parse`, `/documents/batch` envelope, and provisional `/documents/fraud-status/{job_id}` top-level baselines. `official-openapi.json` now has conservative observed-runtime alignment for fraud-status `fraudJobId`, `fraudStatus`, and `404`; terminal result fields remain loose.
...
3. Continue observed-runtime characterization for `GET /v1/documents/fraud-status/{job_id}` only beyond the current top-level OpenAPI alignment when additional safe sanitized evidence exists. Keep owner confirmation as the gate for public-contract promotion, protected/default-suite inclusion, strict complete-result schemas, auth-negative expansion, artifact policy changes, and long-lived guarantees.
...
- `/parse` OpenAPI drift pilot evidence is documented from a fresh protected report run, `official-openapi.json` now includes optional `pipeline.use_cache` plus a conservative parse success schema, and the aligned shape is guarded by a narrow non-live JSON inspection test. The observed-runtime drift report compares `/parse`, `/documents/batch` envelope, and provisional fraud-status top-level baselines against the checked-in spec without treating observed behavior as owner-approved contract; the current compared baselines are aligned. Non-live validation isolation is also statically guarded.
