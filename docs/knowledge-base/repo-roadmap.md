...
Proposal:
- First implementation tranche, after owner confirmation only: use the async fraud parse path as the likely fresh job-id producer if `pipeline.async_fraud=true` is public/safe for regression setup and staging has reliable job-store support. Characterize `GET /v1/documents/fraud-status/{job_id}` without committing raw payloads, then add minimal status/shape coverage in the opt-in GET smoke lane or a future endpoint-specific opt-in lane.
...
3. For endpoint expansion, obtain owner-backed confirmation for `GET /v1/documents/fraud-status/{job_id}`: whether `pipeline.async_fraud=true` is public/safe for regression setup, tenant config requirements, staging DB/job-store availability, runtime/polling limits, complete-result field stability, artifact policy, and whether opt-in GET smoke is safe or an endpoint-specific opt-in lane is required.
