...
- The repo now has a reusable batch ground-truth export workflow at `tools/reporting/export_batch_ground_truth.py` that reads the shared generated fixture registry, executes `/v1/documents/batch` in safe chunks across discovered fileTypes, supports opt-in bounded fileType-level and per-fileType chunk concurrency with sequential defaults, retries confirmed IAP token expiry and selected transient chunk transport failures once by default, preserves raw batch artifacts, writes full/audit workbooks plus clean GT candidate workbooks, and emits recovery triage JSON/CSV plus manifests under `reports/batch_ground_truth/`.
