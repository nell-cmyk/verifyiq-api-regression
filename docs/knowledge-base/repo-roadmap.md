...
- The repo now has a reusable batch ground-truth export workflow at `tools/reporting/export_batch_ground_truth.py` that reads the shared generated fixture registry, executes `/v1/documents/batch` in safe chunks across discovered fileTypes, supports opt-in bounded fileType-level and per-fileType chunk concurrency with sequential defaults, preserves raw batch artifacts, and writes one Excel workbook per fileType plus a manifest under `reports/batch_ground_truth/`.
