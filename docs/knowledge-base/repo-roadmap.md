...
- External API source evidence from `boost-capital/ai-parser-studio` has refined the non-live design in `docs/knowledge-base/document-processing-adjacent/fraud-status-expansion-plan.md`: fraud job IDs use a strict `fj_` plus 32 lowercase-hex format, job lookup is tenant-scoped through the API key hash, source-backed response shapes exist for `pending`, `running`, `complete`, and `failed`, and malformed/missing/wrong-tenant jobs return `404`.
- Observed in bounded staging characterization on 2026-04-27, with no API owner available: one `/parse` request using the protected fixture and `pipeline.async_fraud=true` returned a format-valid fraud job ID; bounded polling observed `running` then `complete`; invalid-format and artificial valid nonexistent job IDs returned `404`. This is a provisional regression contract only and is not owner-confirmed.

Proposal:
- First implementation tranche, after owner confirmation or maintainer acceptance of the provisional contract: use the async fraud parse path as the likely fresh job-id producer if `pipeline.async_fraud=true` is public/safe for regression setup and staging has reliable job-store support. Keep coverage artifact-free and bounded, then add minimal status/shape coverage in the opt-in GET smoke lane or a future endpoint-specific opt-in lane.
- Intended lane: opt-in GET smoke only if producer setup is safe, bounded, artifact-free, and not long-polling; otherwise use a future endpoint-specific opt-in mapping. Do not add it to the parse-only protected default.
...
3. For endpoint expansion, obtain owner-backed confirmation for `GET /v1/documents/fraud-status/{job_id}` or maintainer acceptance of the provisional regression contract: whether `pipeline.async_fraud=true` is public/safe for regression setup, tenant config requirements, staging DB/job-store availability, runtime/polling limits, complete-result field stability, artifact policy, and whether opt-in GET smoke is safe or an endpoint-specific opt-in lane is required.
...
- Remaining true GET 200-smoke backlog is limited to four still-blocked endpoints: `/v1/admin/cache/stats`, `/monitoring/api/v1/providers`, `/ai-gateway/s3/s3/list`, and `/v1/documents/fraud-status/{job_id}`. Fraud-status now has observed bounded staging evidence but remains blocked pending owner-confirmed producer and contract evidence or maintainer acceptance of the provisional contract; keep exact-status guards for known `401`, `403`, and `502` surfaces. See `docs/operations/endpoint-coverage-inventory.md`.
