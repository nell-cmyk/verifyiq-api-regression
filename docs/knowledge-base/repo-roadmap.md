...
- Current OpenAPI drift comparison treats curated observed endpoint response shape as the runtime baseline against `official-openapi.json`, while keeping public API contract status separate from both observed behavior and the checked-in spec. The non-live `tools/reporting/openapi_runtime_drift.py` report currently compares `/parse`, `/documents/batch` envelope, and provisional `/documents/fraud-status/{job_id}` top-level baselines.
...
3. Continue observed-runtime characterization for `GET /v1/documents/fraud-status/{job_id}` from artifact-free smoke assertions, sanitized summaries, and current repo evidence without treating it as public contract. Keep owner confirmation as the gate for public-contract promotion, protected/default-suite inclusion, strict complete-result schemas, auth-negative expansion, artifact policy changes, and long-lived guarantees.
