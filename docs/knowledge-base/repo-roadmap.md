...
- Current OpenAPI drift comparison treats curated observed endpoint response shape as the runtime baseline against `official-openapi.json`, while keeping public API contract status separate from both observed behavior and the checked-in spec. The non-live `tools/reporting/openapi_runtime_drift.py` report currently compares `/parse` and `/documents/batch` envelope baselines only.
...
- Minimum categories before promoting the group beyond provisional status: owner-confirmed smoke/status signal, conservative contract shape from owner/OpenAPI evidence, auth/validation expectations, and documented negative behavior for invalid or expired job ids.
...
3. For endpoint expansion, obtain owner-backed confirmation for `GET /v1/documents/fraud-status/{job_id}` before promoting beyond provisional smoke coverage: whether hidden/internal `pipeline.async_fraud=true` may be used safely for regression setup or needs a public replacement producer, tenant and `fraud_detection=true` requirements, staging DB/job-store/background-capacity availability, TTL/404 expectations, runtime/polling limits, complete-result field stability, auth-layer behavior, and artifact policy.
...
- `/parse` OpenAPI drift pilot evidence is documented from a fresh protected report run, `official-openapi.json` now includes optional `pipeline.use_cache` plus a conservative parse success schema, and the aligned shape is guarded by a narrow non-live JSON inspection test. The first observed-runtime drift report tranche compares `/parse` and `/documents/batch` envelope baselines against the checked-in spec without treating observed behavior as owner-approved contract. Non-live validation isolation is also statically guarded.
- Remaining true GET 200-smoke backlog is limited to three still-blocked endpoints plus one artifact-policy deferred export endpoint: `/v1/admin/cache/stats`, `/monitoring/api/v1/providers`, `/ai-gateway/s3/s3/list`, and `/monitoring/api/v1/golden-dataset/export`. Fraud-status has maintainer-accepted provisional opt-in smoke coverage but remains blocked from owner-confirmed status pending producer and contract confirmation; keep exact-status guards for known `401`, `403`, and `502` surfaces. See `docs/operations/endpoint-coverage-inventory.md`.
