...
- The canonical operator runner is `tools/run_regression.py`. Current safe discovery output confirms implemented live suites `protected`, `smoke`, and `full`; planned live suite `extended` with non-live dry-run plan support only; implemented endpoint groups `parse` and `batch`; implemented category selections `matrix`, `contract`, `auth`, and `negative`; and planned-but-unmapped category `legacy`.
...
- Structured-report runner mappings are non-targeted and guarded by non-live runner tests: no-arg `--report` and `--suite protected --report` delegate to the baseline report helper, `--suite full --report` delegates to the full wrapper, and `--endpoint parse --category matrix --report` delegates to the matrix wrapper. `--suite extended --dry-run --report` writes a synthetic non-live hub plan report only; it does not execute endpoints or prove live hub reporting.
...
- Automation Hub Expansion is a planned roadmap item with a non-live foundation only, not an implemented live runner mode. The future target is a dependency-aware safe endpoint hub behind `tools/run_regression.py`. `--suite extended --dry-run` is now a non-live dependency-plan preview, can be filtered with `--hub-node` or `--hub-group`, and can write synthetic plan evidence with `--report`; live `--suite extended` execution remains unavailable until endpoint mapping, tests, reporting behavior, docs, and CI posture are implemented and approved.
...
- Keep the planned Automation Hub Expansion behind the canonical runner and live execution future-tense until implemented. The no-argument runner remains the parse-only protected suite, and hub planning must not imply that every endpoint is safe or currently runnable. The `extended` suite currently supports non-live dry-run preview, selector filtering, and synthetic report generation only.
...
Target state: evolve toward one canonical dependency-aware runner for safe, non-legacy VerifyIQ endpoint automation, still reached through `tools/run_regression.py`. The likely future live suite name is `extended`. Today, only `--suite extended --dry-run` is available as a non-live dependency graph and reporting-contract preview; it can be filtered with `--hub-node` or `--hub-group` for bounded discovery, `--report` can write synthetic plan evidence, and live `--suite extended` execution is not implemented.
...
First non-live foundation status: `tools/run_regression.py --suite extended --dry-run` now renders an endpoint-group oriented manifest preview with dependency order, named outputs, dependency skip semantics, artifact policy, and the scaffolded evidence/redaction contract. The preview can be filtered with `--hub-node` or `--hub-group`; filtered output includes required prerequisite closure but remains non-live discovery only and does not prove live endpoint safety. Adding `--report` writes disposable synthetic plan reports under `reports/hub/<run-id>/run.json` and `run.md`; those reports contain no live endpoint results or raw response bodies. This path does not execute endpoints, broaden the protected default, or replace current delegated wrappers.
