...
3. For endpoint expansion, obtain owner-backed confirmation for `GET /v1/documents/fraud-status/{job_id}` before promoting beyond provisional smoke coverage: whether `pipeline.async_fraud=true` is public/safe for regression setup, tenant config requirements, staging DB/job-store availability, runtime/polling limits, complete-result field stability, and artifact policy.
