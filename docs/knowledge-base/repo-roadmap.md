...
First smoke-to-extended candidate tranche status: `docs/operations/endpoint-coverage-inventory.md` now defines a candidate slice for the remaining top-level health probe siblings: `GET /health/live`, `GET /health/detailed`, and `GET /health/startup`. The hub manifest models them as `get-smoke.health.live`, `get-smoke.health.detailed`, and `get-smoke.health.startup` dry-run-only candidate nodes with independent metadata-only artifact expectations. Non-live manifest, report-writer, and runner tests prove candidate metadata, selector behavior, dry-run output, and synthetic report payloads; executor tests prove the candidates are rejected before client creation; report-writer tests reject live reports for those candidates; and runner tests prove rejected live `--report` attempts write no hub reports. This does not approve live execution, approve live `--hub-group`, replace smoke, expand CI, or approve body persistence.

Catalog normalization status: `docs/operations/endpoint-coverage-inventory.md` now has normalized Automation Hub catalog entries for every current endpoint group in the group inventory: `parse`, `batch`, `document-processing-adjacent`, `health`, `applications-api`, `monitoring`, `parser-studio`, `qa`, `admin`, and `other-service-surfaces`. Treat further catalog normalization as maintenance for new evidence or new groups, not an unfinished current tranche.

Next-tranche sequence:
1. Keep catalog normalization current as new evidence or new endpoint groups appear; the current endpoint-group catalog is normalized.
2. Identify the next smoke-covered read-only candidate slice for `extended` without moving current `smoke` coverage, using the top-level health probe sibling tranche as the first documented candidate slice and stopping at approval-dependent boundaries.
3. Continue non-live manifest/reporting proof only for documented candidate slices before any broad live execution. Prove dry-run, selector, dependency, skip, blocked-live, and artifact-contract behavior first.
4. Promote one narrow live-safe tranche at a time through explicit selectors only after approval, keeping `smoke` as the fallback until parity and approval gates are complete.
