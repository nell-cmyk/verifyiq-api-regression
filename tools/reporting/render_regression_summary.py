#!/usr/bin/env python3
"""Render a post-run regression summary artifact from saved terminal output."""
...
def markdown_table(rows: list[EnrichedParseResult]) -> str:
    header = (
...
def candidate_block(result: EnrichedParseResult, today: str, command: str) -> str:
    return "\n".join(
        [
    ...
    *,
    ...
    for row in rows:
        ...
        if row.verification_status != "unverified":
            continue
        candidates.append(candidate_block(row, today=today, command=command))
    return candidates
...
def highlights(rows: list[EnrichedParseResult]) -> list[str]:
    ...
    if failure_classes:
        items.append(f"Failure classes observed: {', '.join(failure_classes)}.")

    if not items:
        items.append("No notable current-run highlights beyond the recorded pass set.")
